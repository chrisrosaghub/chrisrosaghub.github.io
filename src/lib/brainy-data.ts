...
export interface Activity {
  ...
  level?: Level;
  /** A bounded set of grade levels where this activity is appropriate. */
  availableLevels?: Level[];
...
export const SUBJECTS: Subject[] = [
  ...
  },
  {
    ...
    name: "Python Coding",
    emoji: "🐍",
    mascot: "Pixel the Python",
    tagline: "Think in steps, solve problems, and build with code",
    gradientClass: "from-cyan-300 via-sky-300 to-yellow-300",
...
export function isActivityAvailableAtLevel(activity: Activity, level: Level): boolean {
  return activity.allLevels
    || activity.availableLevels?.includes(level)
    || (!activity.availableLevels && (activity.level ?? "grade2") === level);
}

/** Return all activities matching the given level (default: grade2). */
export function getActivitiesForLevel(level: Level): Activity[] {
  return ACTIVITIES.filter((activity) => isActivityAvailableAtLevel(activity, level));
}
...
export function totalActivitiesForSubjectAndLevel(subjectId: SubjectId, level: Level): number {
  return ACTIVITIES.filter((activity) => (
    activity.subjectId === subjectId && isActivityAvailableAtLevel(activity, level)
  )).length;
}
